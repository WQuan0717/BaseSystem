...
def verify_installation(target_dir: Path):
    ...
    required_rules = ["user_rules.md", "project_rules.md"]
...
def print_usage():
    ...
    print("="*60)
    print("\n📚 下一步:")
    print("1. 初始化项目：python .trae/skills/se-context/scripts/init_context.py \"项目名称\"")
    print("2. 开始使用：在 Trae 中 invoke se-lifecycle")
    print("\n📦 核心 Skills:")
...
def cleanup_script():
    ...
    try:
        script_path.unlink()
        print(f"\n🧹 已自动清理安装脚本：{script_path.name}")
    except Exception as e:
        print(f"\n⚠️  未能删除安装脚本：{e}")
        print(f"   可以手动删除：{script_path}")
